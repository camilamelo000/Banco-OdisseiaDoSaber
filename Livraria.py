...
try:
  ...
  cursor.execute("DROP TRIGGER IF EXISTS AtualizaPrecoTotalInsert")
  cursor.execute("DROP TRIGGER IF EXISTS AtualizaPrecoTotalUpdate")

  trigger_pedido_insert = '''
  ...
  END;
  '''
  cursor.execute(trigger_pedido_insert)

  triger_pedido_update = '''
  ...
  END;
  '''
  cursor.execute(triger_pedido_update)
  conexao.commit()
  ...
  cursor.executemany("insert into Comentario(IdUsuario, IdLivro, Estrelas, Comentarios) values(?, ?, ?, ?)", comentario)
  ...
  cursor.executemany("insert into Usuario(NomeUsuario, Email, Senha) values(?, ?, ?)", usuario)
  conexao.commit()


# Consulta para vericar se salvou registros
...
# Teste da Trigger 
  ...
  cursor.executemany("INSERT INTO Carrinho (IdPedido, IdLivro, Quantidade, Preco, PrecoTotal) VALUES (?, ?, ?, ?, ?)", carrinho)
  conexao.commit()
